-- ============================================================================
-- Migration 102: Add hero photo columns to companies
-- ============================================================================
-- Allows companies to select a project photo as their hero/cover image
-- on the professionals discover page.
-- ============================================================================

-- a) Add columns
ALTER TABLE public.companies
  ADD COLUMN IF NOT EXISTS hero_photo_url TEXT DEFAULT NULL,
  ADD COLUMN IF NOT EXISTS hero_photo_project_id UUID DEFAULT NULL
    REFERENCES public.projects(id) ON DELETE SET NULL;

-- b) Recreate mv_professional_summary with hero_photo_url in COALESCE
...
SELECT
  ...
  -- Cover photo: prefer hero_photo_url, then company_photos cover, then first company photo
  (
    SELECT COALESCE(
      c.hero_photo_url,
...
AS $$
BEGIN
  -- Validate: caller owns the company
  IF NOT EXISTS (
  ...
  -- Validate: project is linked to the company
  IF NOT EXISTS (
  ...
  -- Validate: photo belongs to the project
  IF NOT EXISTS (
...
-- d) RPC: clear_company_hero_photo
CREATE OR REPLACE FUNCTION public.clear_company_hero_photo(
  p_company_id UUID
)
RETURNS VOID
LANGUAGE plpgsql
...
GRANT EXECUTE ON FUNCTION public.set_company_hero_photo(UUID, UUID, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.clear_company_hero_photo(UUID) TO authenticated;

COMMENT ON MATERIALIZED VIEW public.mv_professional_summary IS
  'Professional summary with hero photo support. Updated in migration 102.';
