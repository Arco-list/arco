-- Migration: Fix services_offered to use company-level data
-- Description: Update mv_professional_summary to use companies.services_offered instead of professionals.services_offered

-- Drop the existing view
DROP MATERIALIZED VIEW IF EXISTS public.mv_professional_summary CASCADE;

-- Recreate with company services_offered
...
SELECT
  ...
  p.portfolio_url,
  p.languages_spoken,
  c.services_offered, -- CHANGED: Use company's services_offered instead of professional's
  p.created_at,
  p.updated_at,
  -- Company fields
