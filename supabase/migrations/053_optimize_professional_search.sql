-- Migration: Optimise professional search and location facets
-- 1. Rebuild mv_professional_summary with a tsvector column and supporting index.
-- 2. Recreate professional_search_documents view for compatibility.
-- 3. Expose a stable RPC for location facets.
-- 4. Update search_professionals to rely on the indexed tsvector and enforce Plus plan rules.

-- Drop dependent objects ahead of the materialized view recreation
...
SELECT
  ...
  CASE
    WHEN p.hourly_rate_min IS NOT NULL AND p.hourly_rate_max IS NOT NULL THEN p.hourly_rate_min || ' - ' || p.hourly_rate_max || ' EUR/hour'
    ELSE NULL
  ...
  to_tsvector(
    ...
    trim(
      ...
      FROM (
        ...
        COALESCE(p.title, '') || ' ' ||
        COALESCE(p.bio, '') || ' ' ||
        COALESCE(prof.location, '') || ' ' ||
        COALESCE(c.city, '') || ' ' ||
        COALESCE(c.state_region, '') || ' ' ||
        COALESCE(c.country, '') || ' ' ||
        COALESCE(cat.name, '') || ' ' ||
        COALESCE(cat.slug, '') || ' ' ||
...
CREATE INDEX idx_mv_professional_summary_search_vector ON public.mv_professional_summary USING gin(search_vector);

COMMENT ON MATERIALIZED VIEW public.mv_professional_summary IS 'Optimized professional data for listings, including specialty, plan, search vector, and structured location metadata.';
...
-- RPC exposing location facets with cached filters
...
-- Updated search_professionals leveraging the indexed search vector
...
-- Reinstate access restrictions on the materialized view
