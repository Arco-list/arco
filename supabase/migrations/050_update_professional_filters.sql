-- Migration: Update professional filtering capabilities
-- Description: Adds structured location support, enriches professional summary view,
--              and extends search_professionals RPC for multi-select categories/services.

-- Add structured region column for companies (if not already present)
ALTER TABLE public.companies
  ADD COLUMN IF NOT EXISTS state_region TEXT;

-- Drop dependent view before recreating materialized view
DROP VIEW IF EXISTS public.professional_search_documents;

-- Recreate the professional summary materialized view with extended metadata
...
COMMENT ON MATERIALIZED VIEW public.mv_professional_summary IS 'Optimized professional data for listings, including specialty and structured location metadata.';

-- Recreate professional search documents view to reflect new summary definition
...
-- Update search_professionals RPC to support multi-select category & service filters plus structured location
