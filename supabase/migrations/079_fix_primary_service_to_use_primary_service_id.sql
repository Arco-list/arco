-- Migration: Fix mv_company_listings to use primary_service_id
-- Description: Use companies.primary_service_id instead of first services_offered element

-- Drop the existing view
DROP MATERIALIZED VIEW IF EXISTS public.mv_company_listings CASCADE;

-- Recreate using primary_service_id
...
SELECT
  ...
  -- FIXED: Use primary_service_id column (UUID)
  ...
  -- FIXED: Primary service name resolved from primary_service_id
  (
    SELECT cat.name
    FROM categories cat
