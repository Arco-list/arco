-- Migration: Fix mv_company_listings to resolve service UUIDs to names
-- Description: Update materialized view to convert services_offered UUIDs to category names

-- Drop the existing view
DROP MATERIALIZED VIEW IF EXISTS public.mv_company_listings CASCADE;

-- Recreate with service names resolved from UUIDs
...
SELECT
  ...
  c.plan_expires_at,
  c.is_featured,
  -- FIXED: Resolve services_offered UUIDs to category names
  (
  ...
  -- Primary service UUID (first service offered UUID)
  CASE
  ...
  -- FIXED: Primary service name resolved from first service UUID
  (
    CASE
      WHEN c.services_offered IS NOT NULL AND array_length(c.services_offered, 1) > 0 THEN
        (SELECT cat.name FROM categories cat WHERE cat.id::text = c.services_offered[1] LIMIT 1)
      ELSE NULL
    END
