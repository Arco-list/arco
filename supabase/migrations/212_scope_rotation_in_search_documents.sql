-- Scope variation for the featured (default) discover sort: within each
-- (featured-band, scope) group, rank projects by credits then clicks.
-- Ordering by this rank interleaves scopes (new build / interior design /
-- renovation) instead of clustering one scope, while credits stay the
-- dominant signal (the rank is only consulted below is_featured+credits
-- in lib/projects/sort.ts). Appended as a new last column so
