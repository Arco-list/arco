-- ═══════════════════════════════════════════════════════════════════════
-- 133: Surface prospected companies on /professionals
-- ═══════════════════════════════════════════════════════════════════════
-- mv_professional_summary already includes prospected companies (30 rows
-- today) alongside the single listed company. Two RPCs were filtering
-- them out:
--   • search_professionals — `c.status = 'listed'`
--   • count_professionals  — same
--
-- Allowing 'prospected' makes them appear on the discover grid. Two
-- secondary issues surface once we lift the filter:
--
--   1. Prospected companies have no `professionals` rows (no claimed team
--      members yet), so the LEFT JOIN in mv_professional_summary gives us
--      `p.company_id = NULL` for those rows — mv exposes the actual
--      company id under the `company_id_full` alias.
--      Fix: use COALESCE(p.company_id, p.company_id_full) everywhere we
--      need the company id in the RPC.
--
--   2. The credited-sum JOIN keys on `company_id`, so applying the same
--      COALESCE keeps prospected rows joined to their per-company
--      aggregates (and yields 0 for companies not yet on a project).
-- ═══════════════════════════════════════════════════════════════════════
...
COMMENT ON FUNCTION public.search_professionals IS
  'Discover-grid source. sort_by: most_relevant | featured | popular | most_recent. Includes ''listed'' and ''prospected'' companies — the latter need COALESCE(p.company_id, p.company_id_full) because mv_professional_summary''s LEFT JOIN leaves p.company_id null when no team member has claimed the company yet.';

-- ─── count_professionals: match the new status filter ───────────────────
